...
const TopChart = () => {
  ...
  const playListCard = data?.slice(0, 3).map((playlist, index) => {
    return (
      <div
        className="md:w-full min-w-[400px] flex flex-row items-center bg-[#1A1E1F] rounded-3xl md:p-5 sm:p-4 p-3 cursor-pointer mb-4"
        key={playlist.id}
      >
        <div className="flex-1 flex lg:flex-row justify-between md:items-center items-start">
          <div className="lg:flex ">
            <div className="mx-3 lg:mx-0 mb-3 lg:mb-0">
              <img
                className="md:w-20 md:h-20 w-32 h-32 rounded-2xl mr-2"
                src={playlist.cover}
              />
            </div>
